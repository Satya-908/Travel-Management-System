...
DeleteCustomer(){ 
	...
	try { 
		Conn c=new Conn(); 
		ResultSet rs=c.s.executeQuery("Select * from customer where username ='1'"); 
		while(rs.next()) {
...
public void actionPerformed(ActionEvent ae) {
	if(ae.getSource() == b1) {
		try { 
			Conn c=new Conn(); 
			c.s.executeUpdate("delete from account where username ='1'"); 
			c.s.executeUpdate("delete from bookHotel where username ='1'");  
			c.s.executeUpdate("delete from bookpackage where username ='1'");  
			c.s.executeUpdate("delete from customer where username ='1'");  
